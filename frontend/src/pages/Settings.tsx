...
function SettingsRoot({ onNavigate }: { onNavigate: (view: any) => void }) {
  const menus = [
    ...
    },
    {
      ...
      title: '问题分类',
      desc: '管理问题的类型和原因分类',
      icon: ListChecks, // Reusing icon or finding better one
...
function CategorySettings({ onBack }: { onBack: () => void }) {
  const [categories, setCategories] = useState<any[]>([]); // Use appropriate type if available, e.g. Category[]
  const [loading, setLoading] = useState(true);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editName, setEditName] = useState('');
  const [newName, setNewName] = useState('');
